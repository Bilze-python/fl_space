...
def t(key: str) -> str:
    return T.get(key, {}).get(_lang, key)
...
def run(cmd: str | list[str], **kwargs: Any) -> int:
    """运行命令并返回返回码。"""
    print(f"\n  {t('running')}")
...
# ── 菜单类型 ──────────────────────────────────────────────────
MenuItem = tuple[str, str, str]  # (key, label_en, label_zh)
...
def menu(title: str, items: list[MenuItem]) -> str:
    """items: list of (key, label_en, label_zh)"""
    header()
    print(f"  --- {title} ---")
    print()
    for key, en, zh in items:
        label = zh if _lang == "zh" else en
        print(f"    {key}. {label}")
    print()
    print(f"    x. {t('back')}")
    print()
...
def menu_demos() -> None:
    while True:
        ...
        if c == "a":
            run(f"python examples/demo_satellites.py --lang {_lang}")
            pause()
        elif c == "b":
            ...
            run(f"python _run_demo.py --lang {_lang}")
            pause()
        elif c in ("x", ""):
            return
    ...
    while True:
        ...
        if c == "a":
            ...
            code = (
                ...
                "from fl_space.environment import *; from fl_space.orbit import *;"
                "from fl_space.simulator import *; from fl_space.utils.viz import plot_contact_heatmap;"
                ...
                f"plot_contact_heatmap(sim,'{OUTPUT_DIR}/heatmap.png',lang='{_lang}');"
            ...
            code = (
                "import matplotlib; matplotlib.use('Agg');"
                "from fl_space.environment import *; from fl_space.orbit import *;"
                "from fl_space.simulator import *; from fl_space.utils.viz import plot_ground_station_map;"
                "earth=CelestialBody.earth();"
                "orbits=[create_circular_orbit(500,53,0,0,earth)];"
                f"gss=create_default_network({gs});"
                "sim=OrbitSimulator(body=earth,orbits=orbits,ground_station_network=gss,num_timeslots=1440,backend='kepler',verbose=False);"
                f"plot_ground_station_map(sim,'{OUTPUT_DIR}/gs_map.png',lang='{_lang}',show_tracks=True);"
            ...
            code = (
                ...
                "hist=[{'round':i,'accuracy':0.1+0.4*np.tanh(i*0.02)+np.random.normal(0,0.02),'timeslot':i*10} for i in range(100)];"
                f"plot_accuracy_comparison(hist,output_path='{OUTPUT_DIR}/accuracy.png',lang='{_lang}');"
            ...
            code = (
                "import matplotlib; matplotlib.use('Agg');"
                "from fl_space.utils.viz import plot_time_breakdown;"
                "import numpy as np; np.random.seed(42);"
                "hist=[{'round':i,'time_breakdown':{'wait_distribution':np.random.exponential(3),'download':np.random.exponential(2),'train':np.random.exponential(5),'wait_return':np.random.exponential(2),'upload':np.random.exponential(1)}} for i in range(30)];"
                f"plot_time_breakdown(hist,output_path='{OUTPUT_DIR}/time_breakdown.png',lang='{_lang}');"
            ...
            # Heatmap
            run(f'python -c "import matplotlib; matplotlib.use(\\"Agg\\"); from fl_space.environment import *; from fl_space.orbit import *; from fl_space.simulator import *; from fl_space.utils.viz import plot_contact_heatmap; earth=CelestialBody.earth(); orbits=[create_circular_orbit(500,53,0,i*72,earth) for i in range(5)]; gss=create_default_network(5); sim=OrbitSimulator(body=earth,orbits=orbits,ground_station_network=gss,num_timeslots=1440,backend=\\"kepler\\",verbose=False); plot_contact_heatmap(sim,\\"{OUTPUT_DIR}/heatmap.png\\",lang=\\"{_lang}\\"); print(\\"heatmap.png OK\\")"')
            # GS Map
            run(f'python -c "import matplotlib; matplotlib.use(\\"Agg\\"); from fl_space.environment import *; from fl_space.orbit import *; from fl_space.simulator import *; from fl_space.utils.viz import plot_ground_station_map; earth=CelestialBody.earth(); orbits=[create_circular_orbit(500,53,0,0,earth)]; gss=create_default_network(7); sim=OrbitSimulator(body=earth,orbits=orbits,ground_station_network=gss,num_timeslots=1440,backend=\\"kepler\\",verbose=False); plot_ground_station_map(sim,\\"{OUTPUT_DIR}/gs_map.png\\",lang=\\"{_lang}\\",show_tracks=True); print(\\"gs_map.png OK\\")"')
            # Accuracy
            run(f'python -c "import matplotlib; matplotlib.use(\\"Agg\\"); from fl_space.utils.viz import plot_accuracy_comparison; import numpy as np; np.random.seed(42); hist=[{{\\"round\\":i,\\"accuracy\\":0.1+0.4*np.tanh(i*0.02)+np.random.normal(0,0.02),\\"timeslot\\":i*10}} for i in range(100)]; plot_accuracy_comparison(hist,output_path=\\"{OUTPUT_DIR}/accuracy.png\\",lang=\\"{_lang}\\"); print(\\"accuracy.png OK\\")"')
            # Time breakdown
            run(f'python -c "import matplotlib; matplotlib.use(\\"Agg\\"); from fl_space.utils.viz import plot_time_breakdown; import numpy as np; np.random.seed(42); hist=[{{\\"round\\":i,\\"time_breakdown\\":{{\\"wait_distribution\\":np.random.exponential(3),\\"download\\":np.random.exponential(2),\\"train\\":np.random.exponential(5),\\"wait_return\\":np.random.exponential(2),\\"upload\\":np.random.exponential(1)}}}} for i in range(30)]; plot_time_breakdown(hist,output_path=\\"{OUTPUT_DIR}/time_breakdown.png\\",lang=\\"{_lang}\\"); print(\\"time_breakdown.png OK\\")"')
...
def menu_fl() -> None:
    while True:
        ...
        if c == "a":
            ...
            run(f"python examples/quick_test.py --gs {gs} --rounds {rds} --mu {mu} --epochs {eps} {ad_flag} --lang {_lang} --output {OUTPUT_DIR}/quick_test")
            pause()
        elif c == "b":
    ...
    while True:
        ...
        if c == "a":
            ...
            if ask(t("confirm"), "n").lower() == "y":
                run(f"python examples/standard_experiment.py --gs 3 5 7 10 --sats 3 5 7 10 --rounds 300 --lang {_lang} --output {OUTPUT_DIR}/full_grid")
            pause()
        elif c == "b":
            run(f"python examples/standard_experiment.py --gs 1 3 5 --sats 3 5 --rounds 100 --lang {_lang} --output {OUTPUT_DIR}/quick_grid")
            pause()
        elif c == "c":
            gs = ask("Ground Stations", "5")
            ...
            rds = ask("Rounds", "300")
            eps = ask("Epochs", "2")
            run(f"python examples/standard_experiment.py --gs {gs} --sats {sats} --rounds {rds} --epochs {eps} --lang {_lang} --output {OUTPUT_DIR}/single_exp")
            pause()
        elif c == "d":
            gs_list = ask("GS counts (space separated)", "1 3 5")
            rds = ask("Rounds", "300")
            run(f"python examples/run_spacefl_experiment.py --gs-counts {gs_list} --rounds {rds} --lang {_lang} --output {OUTPUT_DIR}/fedprox_suite")
            pause()
        elif c == "e":
            ...
            hrs = ask("Sim Hours", "3")
            run(f"python examples/standard_experiment.py --gs {gs_list} --sats {sat_list} --rounds {rds} --epochs {eps} --lr {lr} --dataset {ds} --altitude {alt} --inclination {incl} --sim-hours {hrs} --lang {_lang} --output {OUTPUT_DIR}/custom_exp")
            pause()
        elif c in ("x", ""):
            return
...
def main() -> None:
    ...
    while True:
        header()
        print(f"    {'1. 快速演示          2. 轨道模拟' if _lang == 'zh' else '1. Quick Demos       2. Orbit Simulation'}")
        print(f"    {'3. 可视化工具        4. FL联邦学习训练' if _lang == 'zh' else '3. Visualization     4. FL Training'}")
        print(f"    {'5. 完整实验套件      6. 调参面板' if _lang == 'zh' else '5. Full Experiment   6. Tune Params'}")
        print(f"    {'7. 配置面板          8. 3D可视化服务器' if _lang == 'zh' else '7. Config Panel      8. 3D Web Server'}")
        print(f"    {'9. 系统信息         10. 切换语言' if _lang == 'zh' else '9. System Info      10. Switch Language'}")
        print(f"    {'0. 退出' if _lang == 'zh' else '0. Exit'}")
        print("  ============================================================")
        c = input(f"\n  {t('select')} [0-10]: ").strip()
